...
public class Laser extends Projectile {
    ...
    private ArrayList<ImageView> damagedAliens = new ArrayList<ImageView>();
    ...
    public Laser(int d, int r, int p, ImageView src,int level ) {
        ...
        super.getImg().setScaleX(0.2);
        super.getImg().setScaleY(0.2);
    ...
    AnimationTimer collisionTimer = new AnimationTimer() {
        ...
        public void handle(long l) {
            for (Alien al : GameController.gc.getAliens()) {
                if (Laser.super.getImg().getBoundsInParent().intersects(al.getImg().getBoundsInParent())) {
                    Laser.this.onHit(al);
                    break;
                }
            }
